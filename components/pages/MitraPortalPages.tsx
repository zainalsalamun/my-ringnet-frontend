...
import { ArrowRight, Banknote, Cable, CircleDollarSign, ClipboardCheck, Clock3, Download, ExternalLink, FilePenLine, FileText, Handshake, Headphones, Landmark, MonitorUp, PackageCheck, Percent, RadioTower, ReceiptText, Router, Server, Ticket, TicketCheck, Timer, Trash2, Upload, UserRoundCheck, Users, Wifi, Wrench } from "lucide-react";
...
export function MitraDashboardPage() {
  ...
  useEffect(() => { api.get("/mitra-portal/summary").then((res) => setData(res.data.data)).catch((err) => setError(err.response?.data?.message || "Gagal memuat dashboard mitra.")); }, []);
...
function DocumentCards({ category }: { category: string }) {
  ...
  useEffect(() => { api.get(endpoint).then((r) => setRows(r.data.data || [])).catch((e) => setError(e.response?.data?.message || "Gagal memuat dokumen.")); }, [endpoint]);
...
function PartnerDocuments({ section }: { section: string }) {
  ...
  const getEndpoint = isAdmin ? `/documents?categorySlug=${category}${popId ? `&popId=${popId}` : ''}` : `/mitra-portal/content-documents?category=${category}`;
  const submitEndpoint = isAdmin ? '/documents' : '/mitra-portal/documents';
  const load = () => api.get(getEndpoint).then((r) => setRows(r.data.data || [])).catch((e) => setError(e.response?.data?.message || "Gagal memuat dokumen."));
  useEffect(() => { load(); }, [getEndpoint]);
  async function submit(e: FormEvent) { e.preventDefault(); if (!file) return setError("Pilih file dokumen terlebih dahulu."); setError(""); const body = new FormData(); Object.entries({ ...form, categorySlug: category, popId }).forEach(([k, v]) => { if (v) body.append(k, v) }); body.append("file", file); try { await api.post(submitEndpoint, body); setMessage("Dokumen berhasil ditambahkan."); setForm({ name: "", documentNo: "", description: "", expiredDate: "" }); setFile(null); await load(); } catch (err: any) { setError(err.response?.data?.message || "Gagal mengunggah dokumen."); } }
  async function remove(id: string) { if (!window.confirm("Hapus dokumen ini?")) return; try { await api.delete(`${submitEndpoint}/${id}`); await load(); } catch (err: any) { setError(err.response?.data?.message || "Gagal menghapus dokumen."); } }
...
function RegistrationPage({ settings = false }: { settings?: boolean }) {
  ...
  useEffect(() => { api.get("/mitra-portal/profile").then((r) => setForm(r.data.data)).catch((e) => setError(e.response?.data?.message || "Gagal memuat profil.")); }, []);
