...
export function CustomerForm({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  useEffect(() => {
    ...
    customerService
      ...
      .catch((err) => setError(formatErrorMessage(err, "Gagal memuat data pelanggan dari database.")));
  }, [edit, id]);
  ...
  async function save() {
    ...
    const dekadataPayload = {
      ...
      type:
        form.customerType === "Perumahan / Apartemen / Kos"
          ? "home"
          : form.customerType.toLowerCase().includes("bisnis")
          ? "business"
    ...
    try {
      if (edit && id) {
        try {
          await customerService.update(id, dekadataPayload);
        } catch {
          await customerService.updateCustomerPartner({
            ...dekadataPayload,
            selectedCustomerId: id,
          });
        }
      } else {
        await customerService.create(dekadataPayload);
      }
      router.push("/users/pelanggan");
    } catch (err: any) {
      setError(formatErrorMessage(err, "Gagal menyimpan pelanggan."));
    }
  }

  return (
    <FormShell
      title={(edit ? "Edit" : "Tambah") + " Pelanggan"}
      subtitle="Lengkapi data pelanggan, akun layanan, lokasi pemasangan, produk, dan dokumen."
      onSubmit={save}
      backHref="/users/pelanggan"
    >
      ...
      <div className="space-y-6">
        <FormSection title="Informasi Pelanggan" description="Identitas dasar pelanggan seperti di master data apps.ring.">
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <TextInput label="Nama Lengkap" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nama pelanggan" required />
            <TextInput label="No. Telepon / HP" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value, whatsapp: form.whatsapp || e.target.value })} placeholder="08xxxxxxxxxx" required />
            <TextInput label="No. WhatsApp" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} placeholder="08xxxxxxxxxx" />
            <TextInput label="Alamat Email" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value, username: form.username || e.target.value.split("@")[0] })} placeholder="pelanggan@domain.com" />
            <SelectInput
              label="Jenis Pelanggan"
              value={form.customerType}
              onChange={(e) => setForm({ ...form, customerType: e.target.value })}
              options={[
                { label: "Perumahan / Apartemen / Kos", value: "Perumahan / Apartemen / Kos" },
                { label: "Bisnis / UKM", value: "Bisnis / UKM" },
                { label: "Korporasi / Enterprise", value: "Korporasi / Enterprise" },
              ]}
            />
            <SelectInput label="Status Pelanggan" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} options={statusOptions} />
          </div>
        </FormSection>

        <FormSection title="Identitas & Legalitas" description="Nomor KTP, NPWP, dan berkas pendukung pelanggan.">
          <div className="grid gap-5 lg:grid-cols-2">
            <TextInput label="Nomor KTP / NIK" value={form.ktp} onChange={(e) => setForm({ ...form, ktp: e.target.value })} placeholder="16 digit NIK" />
            <TextInput label="Nomor NPWP" value={form.npwp} onChange={(e) => setForm({ ...form, npwp: e.target.value })} placeholder="15/16 digit NPWP" />
            <FileInput label="Foto KTP" accept=".pdf,.jpg,.jpeg,.png" fileName={documentFiles.ktp?.name} onChange={(file) => setDocumentFiles((current) => ({ ...current, ktp: file }))} />
            <FileInput label="Foto NPWP" accept=".pdf,.jpg,.jpeg,.png" fileName={documentFiles.npwp?.name} onChange={(file) => setDocumentFiles((current) => ({ ...current, npwp: file }))} />
          </div>
        </FormSection>

        <FormSection title="Akun Layanan Internet" description="Username aplikasi, akun PPPoE, dan kredensial akses jaringan.">
          <div className="grid gap-5 lg:grid-cols-2">
            <TextInput label="Username Aplikasi" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="Username login web/app" />
            <TextInput label="Password Aplikasi" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder={edit ? "Kosongkan jika tidak diubah" : "Password login web/app"} />
            ...
            <TextInput label="Password PPPoE" type="password" value={form.servicePassword} onChange={(e) => setForm({ ...form, servicePassword: e.target.value })} placeholder="Password PPPoE pelanggan" />
            <TextInput label="IP Address" value={form.ipAddress} onChange={(e) => setForm({ ...form, ipAddress: e.target.value })} placeholder="Contoh: 10.10.x.x (opsional)" />
            <TextInput label="Router / NAS" value={form.routerNas} onChange={(e) => setForm({ ...form, routerNas: e.target.value })} placeholder="Contoh: MIKROTIK-CORE-01" />
            <TextInput label="Nama POP" value={form.popName} onChange={(e) => setForm({ ...form, popName: e.target.value })} placeholder="Nama POP" />
            <div className="grid gap-5 sm:grid-cols-2">
              <TextInput label="ODP" value={form.odpName} onChange={(e) => setForm({ ...form, odpName: e.target.value })} placeholder="ODP-xxx" />
              <TextInput label="Port ODP" value={form.port} onChange={(e) => setForm({ ...form, port: e.target.value })} placeholder="Port" />
            </div>
          </div>
        </FormSection>

        <FormSection title="Lokasi Pemasangan" description="Alamat rumah/kantor, area layanan, kota, dan titik koordinat GPS.">
          <div className="grid gap-5 lg:grid-cols-2">
            <TextInput label="Area Layanan" value={form.area} onChange={(e) => setForm({ ...form, area: e.target.value })} placeholder="Contoh: Papringan, Caturtunggal" />
            <TextInput label="Kota / Kabupaten" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} placeholder="Contoh: Sleman, Yogyakarta" />
            <div className="lg:col-span-2">
              <TextInput label="Alamat Lengkap" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="Alamat jalan, nomor rumah, RT/RW" />
            </div>
            <div className="lg:col-span-2">
              <span className="mb-2 block text-sm font-semibold text-slate-700">Titik Koordinat Lokasi</span>
              <div className="flex gap-2">
                <input
                  value={form.coordinate}
                  onChange={(e) => setForm({ ...form, coordinate: e.target.value })}
                  placeholder="-7.77720164, 110.3977788"
                  className="h-11 min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                />
                <button
                  type="button"
                  onClick={() => setCoordinatePickerOpen(true)}
                  className="h-11 rounded-lg border border-indigo-200 px-4 text-sm font-bold text-indigo-600 hover:bg-indigo-50"
                >
                  Pilih Maps
                </button>
              </div>
        ...
        <FormSection title="Paket & Administrasi" description="Paket internet, siklus penagihan, reseller, dan PIC operasional internal.">
          <div className="grid gap-5 lg:grid-cols-2">
            <SelectInput
              label="Paket Layanan"
            ...
            <SelectInput
              ...
              options={[
                ...
                { label: "Tahunan (Yearly)", value: "yearly" },
                { label: "Sekali Bayar (One Time)", value: "one_time" },
              ]}
            />
            ...
            <SelectInput
              label="Mitra / Reseller (Opsional)"
              value={form.partnerId}
              onChange={(e) => setForm({ ...form, partnerId: e.target.value })}
              options={[{ label: "Tanpa Mitra (Langsung)", value: "" }, ...partnerOptions]}
            />
            <SelectInput
              label="Dukungan Pembayaran"
              value={form.supportPayment}
              onChange={(e) => setForm({ ...form, supportPayment: e.target.value })}
              options={adminOptions.length ? adminOptions : [{ label: "Belum ada admin", value: "" }]}
            ...
            />
            <div className="lg:col-span-2">
              <TextArea label="Catatan Tambahan (Opsional)" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Catatan khusus teknis atau administrasi" />
            </div>
          </div>
        </FormSection>
      </div>
...
export function CompanyForm({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  useEffect(() => {
    ...
    companyService
      ...
      .catch((err) => setError(formatErrorMessage(err, "Gagal memuat data bisnis dari database.")));
  }, [edit, id]);
  ...
  async function save() {
    ...
    try {
      ...
      setError(formatErrorMessage(err, "Gagal menyimpan bisnis."));
    }
  }

  return (
    <FormShell
      title={(edit ? "Edit" : "Tambah") + " Pelanggan Bisnis"}
      subtitle="Lengkapi data perusahaan, PIC, legalitas, akun layanan, lokasi, produk, dan billing."
      onSubmit={save}
      backHref="/users/bisnis"
    >
      {error ? <div className="rounded-lg bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{error}</div> : null}
      <div className="space-y-6">
        <FormSection title="Informasi Perusahaan" description="Identitas master pelanggan bisnis/perusahaan.">
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <TextInput
              ...
              value={form.email}
              onChange={(e) => setForm({ ...form, email: e.target.value, username: form.username || e.target.value.split("@")[0] })}
        ...
        <FormSection title="Akun Layanan Internet" description="Akun aplikasi, PPPoE, dan data teknis jaringan pelanggan bisnis.">
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <TextInput
              ...
              placeholder={edit ? "Kosongkan jika tidak diubah" : "Password customer bisnis"}
            />
            <TextInput label="Username PPPoE" value={form.serviceUsername} onChange={(e) => setForm({ ...form, serviceUsername: e.target.value })} placeholder="username@ring.net.id" />
        ...
        <FormSection title="Lokasi Pemasangan" description="Alamat kantor/site, area layanan, kota, dan koordinat pemasangan.">
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <div className="lg:col-span-2">
              <TextInput label="Alamat" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="Alamat bisnis/perusahaan" />
            </div>
            <div className="lg:col-span-2">
        ...
        <FormSection title="Produk, Billing, dan Support" description="Paket layanan bisnis, siklus billing, jadwal aktivasi, dan PIC support internal.">
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            />
            <div className="lg:col-span-2">
              <TextArea label="Catatan" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Catatan tambahan opsional" />
            </div>
          </div>
        </FormSection>
      </div>
...
export function PartnerForm({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  useEffect(() => {
    ...
    partnerService
      ...
      .then((data: any) => {
        if (!data) return;
        setForm((current) =>
          mergeCleanForm(current, {
            ...data,
            loginEmail: data.user?.email || data.email || "",
            agreementStart: data.agreementStart ? String(data.agreementStart).slice(0, 10) : "",
            agreementEnd: data.agreementEnd ? String(data.agreementEnd).slice(0, 10) : "",
            accountPassword: "",
          })
        );
      })
      .catch((err) => setError(formatErrorMessage(err, "Gagal memuat data mitra dari database.")));
  }, [edit, id]);
  ...
  async function save() {
    ...
    try {
      ...
      setError(formatErrorMessage(err, "Gagal menyimpan mitra."));
    }
  }

  return (
    <FormShell
      title={(edit ? "Edit" : "Tambah") + " Reseller / Mitra"}
      subtitle="Buat profil, akun login, legalitas, dan skema pembukuan reseller/mitra."
      onSubmit={save}
      backHref="/users/mitra"
    >
      {error ? <div className="rounded-lg bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{error}</div> : null}
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <TextInput
          label={edit ? "Password Baru (opsional)" : "Password Portal"}
          type="password"
          value={form.accountPassword}
          onChange={(e) => setForm({ ...form, accountPassword: e.target.value })}
          placeholder={edit ? "Kosongkan jika tidak diubah" : "Minimal 8 karakter"}
        />
        ...
        <label className="block">
          ...
          <input
            ...
            onChange={(e) => setDocumentFiles((current) => ({ ...current, ktp: e.target.files?.[0] || null }))}
        ...
        <label className="block">
          ...
          <input
            type="file"
            accept=".pdf,.jpg,.jpeg,.png"
            onChange={(e) => setDocumentFiles((current) => ({ ...current, npwp: e.target.files?.[0] || null }))}
            className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 file:mr-3 file:rounded-md file:border-0 file:bg-indigo-50 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-indigo-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
          />
...
export function LeadForm({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  useEffect(() => {
    ...
    leadService
      ...
      .catch((err) => setError(formatErrorMessage(err, "Gagal memuat data lead dari database.")));
  }, [edit, id]);
  ...
  async function save() {
    ...
    try {
      ...
      setError(formatErrorMessage(err, "Gagal menyimpan lead."));
    }
  }

  return (
    <FormShell title={(edit ? "Edit" : "Tambah") + " Lead"} subtitle="Kelola lead marketing dan assignment ke mitra." onSubmit={save} backHref="/marketing/leads">
      {error ? <div className="rounded-lg bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{error}</div> : null}
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <SelectInput
          ...
          options={partnerOptions.length ? partnerOptions : [{ label: "Belum ada mitra", value: "" }]}
...
export function InvoiceForm({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(
      () => {
        ...
        customerService
          .searchCustomers({ limit: customerSearch.trim() ? 50 : 100, search: customerSearch.trim() }, controller.signal)
          .then((data) => {
    ...
    customerService
      ...
      .then((data: any[]) => {
        const options = data.map((item: any) => ({
          label: item.name,
          value: item.name,
          price: Number(item.monthlyPrice || item.price || 0),
        }));
        setPackageOptions(options);
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    internetServiceService
      ...
      .catch((err) => setError(formatErrorMessage(err, "Gagal memuat faktur dari database.")));
  }, [edit, id]);
  ...
  async function save() {
    ...
    try {
      ...
      const response =
        edit && id
          ? await internetServiceService.update(id, payload)
          : await internetServiceService.create(payload);
      const saved = response?.data?.data || response?.data || response;
      router.push(saved?.id ? `/internet-services/${saved.id}` : "/internet-services");
    } catch (err: any) {
      setError(formatErrorMessage(err, "Gagal menyimpan faktur."));
    }
  }
      ...
      <Card className="overflow-hidden">
        ...
        >
          ...
          <div className="space-y-6 p-5">
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <div className="lg:col-span-2">
                <TextInput label="Nama Tagihan" value={form.invoice_name} onChange={(e) => setForm({ ...form, invoice_name: e.target.value })} placeholder="Contoh: Periode Mei 2026" />
              </div>
              <SelectInput
                label="Jenis Faktur"
                ...
                options={[
                  { label: "Faktur Pelanggan", value: "pelanggan" },
                  { label: "Faktur Umum", value: "umum" },
                  { label: "Faktur Mitra & Bisnis", value: "mitra_bisnis" },
                ]}
              />
              <SelectInput
                label="Pelanggan"
                searchable
                searchPlaceholder="Cari ID pelanggan atau nama..."
                onSearchChange={setCustomerSearch}
                searching={customerLoading}
                value={form.customer_id}
                onChange={(e) => chooseCustomer(e.target.value)}
                options={
                  customerOptions.length
                    ? [{ label: "Pilih pelanggan", value: "" }, ...customerOptions.map(({ label, value }) => ({ label, value }))]
                    : [{ label: customerLoading ? "Memuat pelanggan..." : "Belum ada pelanggan", value: "" }]
                }
                disabled={!customerOptions.length && !customerLoading}
              />
              <SelectInput
                label="Autentikasi"
                searchable
                searchPlaceholder="Cari ID pelanggan atau autentikasi..."
                onSearchChange={setCustomerSearch}
                searching={customerLoading}
                value={form.authentication_id}
                onChange={(e) => setForm({ ...form, authentication_id: e.target.value })}
                options={
                  customerOptions.length
                    ? [{ label: "Pilih autentikasi", value: "" }, ...customerOptions.map(({ label, value }) => ({ label, value }))]
                    : [{ label: customerLoading ? "Memuat autentikasi..." : "Belum ada autentikasi", value: "" }]
                }
                disabled={!customerOptions.length && !customerLoading}
              />
            ...
            <div>
              ...
              <div className="rounded-lg border border-slate-200 bg-white">
                <textarea
                  ...
                  className="min-h-36 w-full resize-y rounded-lg px-3 py-3 text-sm text-slate-900 outline-none placeholder:text-slate-400"
                />
              </div>
            </div>
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <SelectInput
                label="Nama Produk"
                searchable
                searchPlaceholder="Cari paket atau jasa..."
                value={itemDraft.name}
                onChange={(e) => chooseProduct(e.target.value)}
                options={packageOptions.length ? [{ label: "Pilih Nama Produk", value: "" }, ...packageOptions.map(({ label, value }) => ({ label, value }))] : [{ label: "Belum ada produk", value: "" }]}
                disabled={!packageOptions.length}
              />
              <label className="block">
                ...
                <div className="flex h-11 w-full items-center rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100">
                  ...
                  <input
                    inputMode="numeric"
                    value={formatRupiahInput(itemDraft.unitPrice)}
                    onChange={(e) => setItemDraft({ ...itemDraft, unitPrice: parseRupiah(e.target.value) })}
                    placeholder="0"
              ...
              <label className="block">
                ...
                <div className="flex h-11 w-full items-center rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100">
                  ...
                  <input
                    inputMode="numeric"
                    value={formatRupiahInput(itemDraft.discount)}
                    onChange={(e) => setItemDraft({ ...itemDraft, discount: parseRupiah(e.target.value) })}
                    placeholder="0"
                    className="h-full min-w-0 flex-1 bg-transparent outline-none placeholder:text-slate-400"
                  />
                </div>
              ...
              <div className="flex justify-end lg:col-span-2">
                <button
                  type="button"
                  onClick={addItem}
                  className="h-10 rounded-lg border border-slate-200 bg-slate-50 px-4 text-sm font-semibold text-slate-700 transition hover:border-indigo-200 hover:text-indigo-600"
                >
                  Tambahkan Produk
                </button>
              </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] border border-slate-200 text-left text-sm">
                <thead className="bg-slate-50 text-xs font-bold uppercase tracking-wide text-slate-500">
                  <tr>
                    <th className="w-10 border-b border-slate-200 px-3 py-3"></th>
                    <th className="border-b border-slate-200 px-3 py-3">Nama Produk</th>
                    <th className="border-b border-slate-200 px-3 py-3">Jumlah</th>
                    <th className="border-b border-slate-200 px-3 py-3">Potongan Harga</th>
                    <th className="border-b border-slate-200 px-3 py-3">Harga Satuan</th>
                    <th className="border-b border-slate-200 px-3 py-3">Total Harga</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((item, index) => (
                    <tr key={`${item.name}-${index}`} className="border-b border-slate-100">
                      <td className="px-3 py-3">
                        <button type="button" onClick={() => removeItem(index)} className="font-bold text-rose-500">
                          x
                        </button>
                      </td>
                  ...
                  {!items.length ? (
                    <tr>
                      <td colSpan={6} className="px-3 py-8 text-center text-sm font-semibold text-slate-400">
                  ...
                  <tr className="bg-slate-50">
                    ...
                    <td className="px-3 py-3 font-bold">{formatCurrency(subtotal)}</td>
                  </tr>
                  <tr className="bg-slate-50">
                    <td colSpan={5} className="px-3 py-3 text-right font-bold">
                      Potongan Harga
                    </td>
                    <td className="px-3 py-3 font-bold">{formatCurrency(discountTotal)}</td>
                  </tr>
                  <tr className="bg-slate-50">
                    <td colSpan={5} className="px-3 py-3 text-right font-bold">
                      Pajak
                    </td>
                    <td className="px-3 py-3 font-bold">{formatCurrency(taxAmount)}</td>
                  </tr>
                  <tr className="bg-slate-900 text-white">
                    <td colSpan={5} className="px-3 py-4 text-right font-bold">
                      Grand Total
                    </td>
                    <td className="px-3 py-4 font-bold">{formatCurrency(grandTotal)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <label className="flex items-center gap-3 border-t border-slate-100 pt-5 text-sm font-semibold text-slate-600">
              <input
                type="checkbox"
                checked={form.disable_auth_on_due}
                onChange={(e) => setForm({ ...form, disable_auth_on_due: e.target.checked })}
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 pt-5">
              <button
                type="button"
                onClick={() => router.push("/internet-services")}
                className="inline-flex h-10 items-center rounded-lg border border-slate-200 px-4 text-sm font-semibold text-slate-700"
              >
                Batal
              </button>
              <button className="h-10 rounded-lg bg-[#6366F1] px-8 text-sm font-semibold text-white shadow-sm shadow-indigo-200">
                Simpan Faktur
              </button>
            </div>
          </div>
