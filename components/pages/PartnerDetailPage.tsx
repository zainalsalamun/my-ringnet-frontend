...
function CustomerAssignmentModal({ partnerId, onClose, onSaved }: { partnerId: string; onClose: () => void; onSaved: (detail: PartnerDetail) => void }) {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      setLoading(true);
      setError("");
      partnersApi.availableCustomers(partnerId, { search: query, limit: 50 })
  ...
  function save() {
    ...
    setSaving(true);
    setError("");
    partnersApi.attachCustomers(partnerId, selected)
      .then((res) => {
...
export default function PartnerDetailPage({ id }: { id: string }) {
  ...
  function detachCustomer(customer: any) {
    ...
    partnersApi.detachCustomer(id, customer.id)
      .then((res) => {
  ...
  function decideRegistration(decision: "approved" | "rejected") {
    ...
    partnersApi.registrationDecision(id, { decision, notes })
