...
import { Check, Edit3, Wallet, X } from "lucide-react";
...
export function ReportsCrudPage() {
  ...
  return (
    <div>
      <PageHeader title="Laporan" subtitle="CRUD template laporan, periode, status publikasi, dan catatan." actionHref="/laporan/new" actionLabel="Tambah Laporan" />
      <Toast message={toast} />
...
export function FinanceFormPage({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  return <CrudForm title={(edit ? "Edit" : "Tambah") + " Pembayaran"} subtitle="Catat transaksi pembayaran dan rekonsiliasi." error={error} onSubmit={submit} back="/keuangan">
    ...
    <SelectInput label="Metode" value={form.method} onChange={(e) => setForm({ ...form, method: e.target.value })} options={[{ label: "Transfer Bank", value: "Transfer Bank" }, { label: "Virtual Account", value: "Virtual Account" }, { label: "Cash", value: "Cash" }, { label: "QRIS", value: "QRIS" }]} />
