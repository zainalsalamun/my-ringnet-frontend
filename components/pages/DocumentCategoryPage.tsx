...
import { Card, PageHeader, TableSkeleton } from "@/components/ui/AdminUI";
...
export function DocumentCategoryPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editId) {
        await documentsApi.updateCategory(editId, { name: formName });
        setToast("Kategori berhasil diperbarui.");
      } else {
        await documentsApi.createCategory({ name: formName });
  ...
  async function executeDelete() {
    ...
    try {
      await documentsApi.removeCategory(deleteConfirm.id);
