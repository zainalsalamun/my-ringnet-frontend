...
function CustomerAssignmentModal({
  ...
  partnerId: string;
  onClose: () => void;
  onSaved: (detail: PartnerDetail) => void;
}) {
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      ...
      setError("");
      partnerService
        .getAvailableCustomers(partnerId, query, 50)
        .then((res) => setRows(res))
        .catch((err) => {
          setRows([]);
          setError(formatErrorMessage(err, "Gagal memuat pelanggan."));
        })
  ...
  function save() {
    ...
    setError("");
    partnerService
      .assignCustomers(partnerId, selected)
      .then((data) => {
        onSaved(data);
        onClose();
      })
      .catch((err) => setError(formatErrorMessage(err, "Gagal menambahkan pelanggan ke marketing.")))
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4">
      <div className="w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4">
          ...
          <button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-lg text-slate-400 hover:bg-slate-50 hover:text-slate-700">
            <X size={18} />
          </button>
        </div>
        <div className="p-5">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Cari ID pelanggan, nama, nomor telepon, area..."
              className="h-11 w-full rounded-lg border border-slate-200 bg-slate-50 pl-10 pr-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-100"
              autoFocus
            />
          </div>
          ...
          <div className="mt-4 max-h-[420px] overflow-y-auto rounded-xl border border-slate-200">
            {loading ? (
              <div className="flex items-center justify-center gap-2 px-4 py-12 text-sm font-semibold text-slate-400">
                <Loader2 size={18} className="animate-spin" /> Memuat pelanggan...
              </div>
            ) : rows.length ? (
              rows.map((customer) => {
                ...
                return (
                  <button
                    ...
                    className={
                      "flex w-full items-start gap-3 border-b border-slate-100 px-4 py-3 text-left transition last:border-b-0 " +
                      (active ? "bg-indigo-50" : "hover:bg-slate-50")
                    }
                  >
                    <span
                      className={
                        "mt-1 grid h-5 w-5 shrink-0 place-items-center rounded border text-xs font-black " +
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-1 block text-sm text-slate-500">
                        {customer.phone || "-"} · {customer.area || "-"} · {customer.city || "-"}
                      </span>
                      <span className="mt-1 block text-xs font-semibold text-slate-400">
                        {customer.packageName || "Paket belum diisi"} · {ownerLabel(customer)}
                      </span>
                    </span>
                  </button>
                );
        ...
        <div className="flex flex-col gap-3 border-t border-slate-100 bg-slate-50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex justify-end gap-3">
            <button onClick={onClose} className="h-10 rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-700">
              Batal
            </button>
            <button
              onClick={save}
              disabled={saving || !selected.length}
              className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#6366F1] px-4 text-sm font-semibold text-white shadow-sm shadow-indigo-200 disabled:opacity-50"
            >
...
export default function PartnerDetailPage({ id }: { id: string }) {
  ...
  function loadDetail() {
    ...
    partnerService
      ...
      .catch((err) => {
        setDetail(null);
        setToast(formatErrorMessage(err, "Gagal memuat detail mitra."));
      })
      .finally(() => setLoading(false));
  }
  ...
  function detachCustomer(customer: any) {
    ...
    partnerService
      ...
      .catch((err) => setToast(formatErrorMessage(err, "Gagal melepas pelanggan.")));
  }
  ...
  function decideRegistration(decision: "approved" | "rejected") {
    ...
    const notes =
      decision === "rejected"
        ? window.prompt("Alasan penolakan:") || "Dokumen atau data belum memenuhi persyaratan."
        : "Pengajuan dan dokumen telah diverifikasi.";
    partnerService
      .decideRegistration(id, { decision, notes })
      .then((res) => {
        setDetail(res.data);
        setToast(res.message || "Pengajuan mitra berhasil diproses.");
      })
      .catch((err) => setToast(formatErrorMessage(err, "Gagal memproses pengajuan Mitra.")));
  }
    ...
    return (
      <div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-3">
          <TableSkeleton columns={3} rows={1} showHeader={false} />
        </div>
  ...
  return (
    <div>
      <PageHeader
        title={detail.name}
        subtitle="Detail marketing/mitra individual, pelanggan terdaftar, faktur & tagihan, dan tiket operasional."
        actionHref={`/users/mitra/${detail.id}/edit`}
        actionLabel="Edit Mitra"
      />
      ...
      {detail.registrationSource === "self_service" ? (
        <Card
        ...
        >
          <div className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-wider text-indigo-600">Registrasi Mandiri Reseller / Mitra</p>
              <h2 className="mt-1 text-lg font-black text-slate-950">
                Status: <span className="uppercase">{detail.status}</span>
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                Diajukan {date(detail.submittedAt)} · Persetujuan ketentuan {date(detail.termsAcceptedAt)}
              </p>
              {detail.reviewNotes ? <p className="mt-2 text-sm font-semibold text-slate-700">Catatan: {detail.reviewNotes}</p> : null}
            </div>
            {detail.status === "pending" ? (
              <div className="flex gap-3">
                <button
                  onClick={() => decideRegistration("rejected")}
                  className="inline-flex h-10 items-center gap-2 rounded-lg border border-rose-200 px-4 text-sm font-bold text-rose-600 hover:bg-rose-50"
                >
                  <XCircle size={17} /> Tolak
                </button>
                <button
                  onClick={() => decideRegistration("approved")}
                  className="inline-flex h-10 items-center gap-2 rounded-lg bg-emerald-600 px-4 text-sm font-bold text-white hover:bg-emerald-500"
                >
                  <CheckCircle2 size={17} /> Setujui & Aktifkan
                </button>
              </div>
            ) : null}
          </div>
      ...
      <div className="mb-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <Card className="overflow-hidden">
          ...
          <div className="p-5">
            ...
            <InfoRow label="Area / Coverage" value={detail.area} />
            <InfoRow label="Kota" value={detail.city} />
            <InfoRow label="Terdaftar Sejak" value={date(detail.createdAt)} />
          </div>
        </Card>
        ...
        <Card className="p-5">
          <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-base font-bold text-slate-950">Pelanggan di Bawah Mitra</h2>
              <p className="text-xs text-slate-500">Daftar pelanggan aktif dan billing yang terafiliasi.</p>
            </div>
            <button
              type="button"
              onClick={() => setAssignOpen(true)}
              className="inline-flex h-9 items-center gap-2 rounded-lg bg-indigo-600 px-3 text-xs font-bold text-white shadow-sm shadow-indigo-200 hover:bg-indigo-500"
            >
          ...
          <DataTable
            ...
            columns={[
              { key: "customerCode", header: "ID", render: (row) => <span className="font-bold text-indigo-600">{row.customerCode || row.id}</span> },
              { key: "name", header: "Nama Pelanggan", render: (row) => <span className="font-semibold text-slate-900">{row.name}</span> },
              { key: "packageName", header: "Paket Layanan", render: (row) => row.packageName || row.package_name || "-" },
              { key: "status", header: "Status", render: (row) => <Badge value={row.status || "active"} /> },
            ]}
            extraActions={(row) => (
              <button
                type="button"
                onClick={() => detachCustomer(row)}
                className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-rose-500 hover:border-rose-200 hover:bg-rose-50"
                title="Lepas dari marketing"
              >
                <Trash2 size={15} />
              </button>
            )}
      ...
      <div className="space-y-6">
        <div>
          <h2 className="mb-4 text-xl font-bold text-slate-900">Riwayat Faktur Pelanggan</h2>
          <DataTable
            ...
            columns={[
              ...
              { key: "customerName", header: "Pelanggan", render: (row) => row.customerName || row.customer?.name || "-" },
      ...
      {assignOpen ? (
        <CustomerAssignmentModal
          ...
          onSaved={(nextDetail) => {
            ...
            setToast("Pelanggan berhasil ditambahkan ke marketing ini.");
          }}
        />
      ) : null}
    </div>
