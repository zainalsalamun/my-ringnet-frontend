...
export default function PopPage() {
  ...
  const load = () => {
    ...
    popService
      ...
      .catch((err) => {
        setRows([]);
        setToast(formatErrorMessage(err, "Gagal memuat data POP dari server."));
      })
  ...
  async function remove(row: any) {
    try {
      await popService.delete(row.id);
      setRows((current) => current.filter((item) => item.id !== row.id));
      setToast("POP berhasil dihapus.");
    } catch (err: any) {
      setToast(formatErrorMessage(err, "Gagal menghapus POP."));
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title="POP"
        subtitle="Kelola wilayah Point of Presence, area layanan, koordinat, dan status jaringan."
        actionHref="/users/pop/new"
        actionLabel="Tambah POP"
      />
      <Toast message={toast} />
      {loading ? null : (
          ...
          columns={[
            {
              ...
              header: "ID POP",
              render: (row: any) => (
                <Link href={`/users/pop/${row.id}/edit`} className="font-semibold text-indigo-600 hover:underline">
                  {row.popCode || "-"}
                </Link>
              ),
            },
...
export function PopFormPage({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  useEffect(() => {
    ...
    popService
      ...
      .then((data: any) => {
        if (!data) return;
        setForm({
          popCode: data.maps_id || data.code || data.popCode || "",
          name: data.name || "",
      ...
      .catch((err) => setError(formatErrorMessage(err, "Gagal memuat data POP.")));
  }, [edit, id]);
  ...
  async function submit() {
    ...
    try {
      if (edit && id) {
        await popService.update(id, dekadataPayload);
      } else {
        await popService.create(dekadataPayload);
      }
      router.push("/users/pop");
    } catch (err: any) {
      setError(formatErrorMessage(err, "Gagal menyimpan POP."));
    }
  }
  ...
  return (
    <div>
      <PageHeader
        title={edit ? "Data PIC" : "Tambah POP"}
        subtitle="Lengkapi data wilayah POP, alamat, koordinat, status jaringan, serta kelengkapan dokumen."
      />
      <Card className="p-6">
        ...
        >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {existingUrl ? (
                        <button
                          ...
                          className={`inline-flex h-[42px] shrink-0 items-center gap-2 rounded-lg border px-4 text-sm font-bold shadow-sm transition ${
                            previewOpen[f.key] ? "border-slate-300 bg-slate-100 text-slate-700 hover:bg-slate-200" : "border-indigo-200 bg-white text-indigo-600 hover:bg-indigo-50"
                          }`}
                        >
                        ...
                        <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                          ...
                          <div className="bg-slate-100 p-1">
                            {isImage ? (
                              ...
                              <iframe src={fileUrl + "#toolbar=1&navpanes=0"} title={"Preview: " + f.label} className="h-[400px] w-full rounded-lg border-0 bg-white" />
                            )}
                          </div>
                        </div>
                      );
            ...
            <div className="lg:col-span-2 mt-4">
              ...
              <div className="mb-4">
                <input
                  ...
                  className="h-11 w-full md:w-1/2 rounded-lg border border-slate-200 bg-white px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"
                />
              </div>
              <div className="w-full md:w-[500px] overflow-hidden rounded-xl border border-slate-200 shadow-sm">
                <CoordinatePicker inline hideSearch={true} readonly={true} value={form.coordinate} />
              </div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-200 pt-5">
            <button type="button" onClick={() => router.push("/users/pop")} className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">
              Batal
            </button>
